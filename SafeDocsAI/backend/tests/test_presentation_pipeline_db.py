...
class FailureTests(PresentationPipelineTestCase):
    ...
    async def test_a_hanging_model_times_out_and_leaves_nothing_behind(self):
        ...
        presentation_job_timeout(SLIDE_COUNT_MIN) — десятки минут): если бы
        зависший вызов ждал его, тест не уложился бы и в час. Снят же заказ
        за доли секунды — то есть по бюджету ОДНОГО вызова. Ровно это и было целью
        ...
        # висящий вызов заготовлен на 30 с, а потолок джобы — десятки минут
        # (presentation_job_timeout ниже подставляется в текст отказа).
        self.assertLess(
            elapsed,
            10.0,
