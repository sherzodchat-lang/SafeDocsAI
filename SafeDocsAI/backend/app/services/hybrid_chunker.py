...
# «ѐ» (U+0450) -> «ё»: артефакт текстового слоя PDF, см. _normalize_blocks.
# Продублировано из text_utils сознательно: чанкер не должен тянуть модуль
# rag ради двух символов, а таблица из них меняться не будет.
...
class HybridChunker:
    ...
    def _normalize_blocks(self, blocks: List[TextBlock]) -> List[TextBlock]:
        ...
        for b in blocks:
            text = b.text
            # Текстовый слой государственных PDF кодирует «ё» как «ѐ» (U+0450):
            # такой чанк не находится ни лексикой (токенизатор букву не знает),
            # ни точным совпадением фразы. Сворачиваем при извлечении, чтобы в
            # базу и в индекс попадала настоящая буква; уже проиндексированные
            # документы это исправит только переиндексация.
