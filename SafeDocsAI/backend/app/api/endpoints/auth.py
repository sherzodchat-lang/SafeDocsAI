...
async def _issue_tokens(
    ...
    return {
        ...
        # Имя и роль владельца выданной пары. Это не секрет — пользователь и
        # так знает, кто он, — и не право доступа: права по-прежнему проверяет
        # сервер на каждом запросе. Клиенту это нужно только чтобы не рисовать
        # разделы, которыми он всё равно не сможет воспользоваться. Отдаём
        # здесь, а не отдельной ручкой /me: значение живёт ровно столько же,
        # сколько выданный access-токен, и обновляется тем же обменом.
