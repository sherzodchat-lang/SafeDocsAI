...
class TopicRead(BaseModel):
    ...
    Подписей несколько, и ни одна не лишняя. label — английское имя темы из
    обучающего корпуса: оно устойчиво, по нему сверяются отчёты эксперимента и
    назначения прошлых версий, и оно есть всегда. label_ru и label_tg — то же
    имя на языках интерфейса, и именно их показывает продукт: экраны переведены
    на русский и таджикский, английского интерфейса нет вовсе. null означает,
    что имени на этом языке у кластера в модели нет, а не что оно пустое, —
    клиент откатывается к label.

    Выбор остаётся за клиентом намеренно: API не знает ни языка интерфейса, ни
    того, что подпись темы передаётся между экранами параметром фильтра и
    обязана называть себя одинаково.
    """

    cluster_index: int
    label: str
    label_ru: str | None = None
    label_tg: str | None = None
    document_count: int
    share: float
...
async def read_topics(
    ...
    Отдаются ВСЕ кластеры активной модели, включая те, в которых сейчас ноль
    документов. Это не недосмотр: полный список — единственный ответ на вопрос
    «что модель вообще различает», он же делает сопоставимыми распределения по
    двум блокнотам (иначе у них разные наборы строк). Обычному пользователю
    двадцать строк, из которых семнадцать пустые, не нужны — и прячет их
    ЭКРАН, свернув пустые темы под раскрытие (frontend/src/pages/TopicsPage.jsx).
    Фильтрация на сервере отобрала бы данные у всех, включая администратора.
    """
    ...
    return [
        TopicRead(
            ...
            label=row.label,
            label_ru=row.labels_localized.get("ru"),
            label_tg=row.labels_localized.get("tg"),
