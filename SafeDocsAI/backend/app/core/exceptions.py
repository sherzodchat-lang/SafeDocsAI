...
class SourceErrors:
    ...
    # В PDF есть страницы-сканы, а OCR на сервере недоступен (нет
    # tesseract/poppler). Отдельный код, а не TEXT_EXTRACTION_FAILED: тот
    # означает «файл не читается», а здесь файл в порядке — не хватает
    # системных пакетов, и чинится это администратором, а не пользователем.
    # Раньше такие страницы молча выпадали из индекса при статусе 'indexed'.
...
class PresentationErrors:
    ...
    #
    # И им же — выдача превью самой КОЛОДЫ, когда картинку из файла получить не
    # удалось: колода прежнего рендерера лежит в .pptx, страницы которого нечем
    # прочитать, или PDF повреждён. Довод третий раз тот же: показать нечего, а
    # различать «файла нет» и «файл есть, но не рисуется» некому — сетка на
    # любой отказ рисует ту же заглушку, а сама колода остаётся скачиваемой.
