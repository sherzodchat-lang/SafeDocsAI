...
class PresentationErrors:
    ...
    Здесь только коды ПАЙПЛАЙНА: они попадают не в HTTP-ответ, а в
    presentation.error_code, и клиент показывает по ним свой перевод рядом со
    статусом 'error'. Коды HTTP-слоя (не найдено, чужая презентация, файл ещё
    не готов) заводятся вместе с самим эндпоинтом — заранее объявленный код,
    которым никто не отвечает, невозможно ни проверить, ни перевести.
    """
