...
class DocumentBase(SQLModel):
    ...
    # --- Тема документа -------------------------------------------------
    #
    # Три колонки, а не одна, и ни одну из них нельзя вывести из остальных.
    #
    # topic_cluster_index — номер кластера обученной модели. Сам по себе он не
    # тема: номер осмыслен только вместе с моделью, которая его выдала.
    #
    # topic_label — подпись ХРАНИТСЯ, а не собирается на лету из активной
    # модели. Иначе переименование кластера (или переобучение) задним числом
    # меняло бы подпись у документов, размеченных прошлой моделью, — то есть
    # переписывало бы историю. Цена — денормализация на одну строку; выгода —
    # то, что пользователь видел вчера, он видит и сегодня.
    #
    # topic_model_version — версия модели, которая это назначение сделала.
    # Ради неё всё и затевалось: без привязки к версии переобучение молча
    # смешивало бы старые назначения с новыми в одном распределении, и
    # «документов в теме N» стало бы числом, не означающим ничего. Раздел тем
    # считает только назначения АКТИВНОЙ версии, а несовпадающие остаются на
    # документе как история и ждут переразметки (POST /topics/reassign).
    #
    # topic_label_ru и topic_label_tg — та же подпись на языках интерфейса, и
    # хранятся они по той же причине, что английская: подпись принадлежит
    # НАЗНАЧЕНИЮ, а не активной модели. Собирать её на лету по
    # topic_cluster_index значило бы у документа, размеченного прошлой версией,
    # показать название темы от текущей — рядом с английским названием от
    # прошлой.
    #
    # Колонка на язык, а не JSON: это денормализованные значения ДЛЯ ПОКАЗА,
    # каждое читается запросом наравне с topic_label, и набор их закрыт
    # продуктом — интерфейс переведён ровно на два языка. Английского
    # интерфейса нет вовсе, поэтому topic_label здесь не «ещё один язык», а
    # устойчивый ключ темы.
    #
    # None — нормальное состояние: перевода может не быть у самой модели
    # (артефакт обучен до его появления), и документы, размеченные до появления
    # колонок, тоже приходят с None. И то и другое клиент читает как
    # «показывай topic_label», а не как ошибку.
    topic_cluster_index: Optional[int] = None
    topic_label: Optional[str] = None
    topic_label_ru: Optional[str] = None
    topic_label_tg: Optional[str] = None
    topic_model_version: Optional[int] = None
...
class TopicModelVersionBase(SQLModel):
    """Обученная модель тем — та её часть, которую надо знать без файла.

    Сама модель (центроиды и преобразование) живёт в артефакте на диске:
    матрицу 4096-мерных центроидов в PostgreSQL класть незачем, читает её
    только слой назначения тем и всегда целиком. В базе лежит РЕЕСТР —
    ответ на вопросы «какая модель сейчас активна», «когда её обучили» и
    «чем размечен вот этот документ», то есть ровно то, что должно пережить
    перезапись файла.

    Версия — целое, растущее на единицу. Она и есть связь с документом
    (Document.topic_model_version). Хранить вместо неё путь к файлу или его
    дату нельзя: файл перезаписывается по тому же пути, и все назначения разом
    стали бы выглядеть сделанными новой моделью.
    """

    # Растущий номер. Уникален не колонкой, а тем, что выдаётся как max + 1
    # внутри одной транзакции; индекс нужен для поиска назначений по версии.
    version: int = Field(index=True)
    # k, с которым обучали, и число центроидов в артефакте. Обычно совпадают,
    # но не обязаны: k — параметр обучения, cluster_count — то, что реально
    # лежит в файле, и расхождение между ними должно быть видно, а не
    # усредняться в одно «k».
    k: int
    cluster_count: int
    # Модель эмбеддингов, которой считали векторы обучающего корпуса. Вектор
    # от другой модели лежит в другом пространстве (а иногда и другой длины),
    # и «ближайший центроид» для него — число без смысла. Сверяется с активной
    # embedding-моделью перед каждым назначением.
    embedding_model: str
    # Описание преобразования вектора ПЕРЕД сравнением с центроидами, строкой
    # для человека и для API (например 'group_mean_shift(language)').
    # Кластеризация «в лоб» на этом корпусе делит его по языку, поэтому
    # победивший вариант вычитает среднее по языку — и слой назначения обязан
    # повторить ровно это, иначе он сравнивает сырой вектор с центроидами
    # преобразованного пространства и получает правдоподобный мусор.
    transform: str = Field(default="none")
    # Метрики защиты (ari_topic, purity, silhouette) — JSON, потому что набор
    # чисел принадлежит эксперименту, а не схеме БД: колонка на каждую метрику
    # означала бы миграцию на каждую новую.
    metrics_json: Optional[str] = None
    # Подписи кластеров на момент РЕГИСТРАЦИИ модели: {"0": "Налоги", ...}.
    # Копия того, что лежит в артефакте, и живёт она здесь потому, что список
    # тем обязан отвечать и тогда, когда файл недоступен или уже перезаписан.
    labels_json: str = Field(default="{}")
    # Подписи тех же кластеров на языках интерфейса:
    # {"ru": {"0": "Налоги"}, "tg": {"0": "Андозҳо"}}. Здесь только те
    # кластеры, у которых перевод ЕСТЬ, — отсутствие ключа означает «в этой
    # модели названия на этом языке нет, показывай labels_json». Заполнять
    # пропуски английским нельзя: тогда клиент не отличил бы перевод от его
    # отсутствия.
    #
    # Одной колонкой на все языки, в отличие от document: там значения читаются
    # запросом на каждый источник, здесь же это справочник на всю модель,
    # который всё равно разбирается целиком. Новый язык интерфейса не должен
    # стоить миграции.
    #
    # Nullable, в отличие от labels_json: колонка появилась позже, и на уже
    # зарегистрированных версиях модели её значения нет и взяться ему неоткуда.
    labels_localized_json: Optional[str] = Field(default="{}")
    artifact_path: str
    # sha256 файла. По нему (а не по mtime) распознаётся, что артефакт
    # переобучили: mtime меняет и обычное копирование, а перерегистрация модели
    # обесценивает все назначения разом.
    artifact_digest: str
    trained_at: datetime = Field(default_factory=utcnow)
    # Активна ровно одна строка. Держит это частичный уникальный индекс
    # TOPIC_MODEL_ACTIVE_INDEX (app/core/database.py) — тем же приёмом, что
    # «одна активная генерация на блокнот» у презентаций.
    is_active: bool = Field(default=True)


class TopicModelVersion(TopicModelVersionBase, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    created_at: datetime = Field(default_factory=utcnow)
