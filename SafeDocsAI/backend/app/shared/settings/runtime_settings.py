...
class RuntimeSettingsService:
    ...
    def _settings_path(cls) -> Path:
        """Путь к файлу настроек. Только вычисление — ничего не создаёт.

        Здесь стоял `path.parent.mkdir(parents=True, exist_ok=True)`, и он
        отрабатывал на КАЖДОМ обращении к пути. Обращений этих столько же,
        сколько чтений настроек: get_settings зовут чат на каждом сообщении,
        индексация на каждом чанке и сам экран настроек на каждом GET — то есть
        лишний сисколл висел на пути каждого запроса.

        Хуже цены — побочный эффект: чистая операция чтения меняла файловую
        систему. На томе, смонтированном только для чтения, GET
        /api/v1/settings/ падал на mkdir ещё до того, как что-то было
        прочитано, — хотя читать там нечего и правильный ответ известен заранее
        (умолчания, ровно как на чистой установке).

        Каталог создаётся там, где он и нужен, — при записи (см.
        _write_settings). Чтению отсутствующий каталог не мешает: path.exists()
        отвечает False, и get_settings идёт той же веткой, что и при
        отсутствующем файле.
        """
        backend_dir = Path(__file__).resolve().parents[3]
        return backend_dir / "data" / "runtime_settings.json"
    ...
    def _write_settings(cls, data: dict[str, Any]) -> None:
        ...
        """
        path = cls._settings_path()
        # Каталог создаётся здесь, а не в _settings_path: это единственное
        # место, которому он обязан существовать (mkstemp ниже кладёт временный
        # файл именно в него), и единственное, где создание каталога — не
        # побочный эффект чтения. Записей единицы, чтений — на каждый запрос.
    ...
    def _normalize_bool(cls, value: Any, default: bool, field: str = "flag") -> bool:
        ...
        if value is None:
            return default
        # Ни истина, ни ложь. Что имел в виду написавший — неизвестно, и
        # угадывать нельзя.
        #
        # Здесь стояло `bool(value)`, а bool непустой строки — это True: "нет",
        # "disabled", "выключено", "нет-нет" читались как ВКЛЮЧЕНО. То есть
        # значение, которым админ пытался функцию выключить, её включало —
        # причём именно тяжёлые функции (reranker_enabled,
        # contextual_embedding_enabled), которые гоняют модель на каждый запрос
        # и на каждый чанк при индексации. Самый дорогой из возможных исходов
        # выпадал на самую вероятную опечатку.
        #
        # Берём `default` — умолчание ЭТОГО поля, то есть ровно то значение, с
        # которым система работает, когда ключа в файле нет вовсе. Почему
        # именно оно, а не безусловное False: у полей умолчания разные и каждое
        # обосновано отдельно (enable_condense_query=True — функция намеренно
        # включена, reranker_enabled=False — намеренно выключена). Глобальное
        # False чинило бы одно тихое включение ценой другого тихого выключения.
        # «Непонятное значение» и «значения нет» — это одно и то же состояние
        # знания, и вести себя они обязаны одинаково.
        #
        # Молча это не проходит: строка в журнале называет поле, само значение
        # и то, как оно прочитано (_report_read_fallback), — через API такое
        # значение отвергается (_require_bool), а в файле оно откуда-то взялось.
        cls._report_read_fallback(field, value, default)
        return default
