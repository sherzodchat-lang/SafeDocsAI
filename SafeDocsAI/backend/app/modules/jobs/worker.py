        ...
        try:
            async with session_context() as session:
                ...
                if doc is not None:
                    ...
                    session.add(doc)
                    await session.commit()
                    # Тема назначается ПОСЛЕ того, как документ уже помечен
                    # проиндексированным, и ничем не может этому помешать:
                    # assign_after_indexing не бросает наружу ничего вообще.
                    #
                    # Порядок и молчаливость здесь принципиальны. Тема —
                    # украшение документа, а поиск по нему — основная функция
                    # продукта. Необученная модель тем, недоступная ChromaDB и
                    # незаданная embedding-модель не имеют права превратить
                    # успешную загрузку файла в 'error': пользователь потерял
                    # бы работающий документ из-за отсутствующей подписи.
                    # Документ без темы получит её при переразметке
                    # (POST /api/v1/topics/reassign).
                    #
                    # Импорт локальный: раздел тем тянет numpy и артефакт
                    # модели, и платить за них при импорте воркера незачем —
                    # без обученной модели этот путь не доходит даже до чтения
                    # файла.
                    # try/except здесь ВТОРОЙ раз, поверх обещания сервиса
                    # ничего не бросать, и это не перестраховка. Обещание
                    # держится кодом соседнего раздела, а цена его нарушения —
                    # успешно проиндексированный документ, помеченный 'error'
                    # из-за отсутствующей подписи. Один except дешевле такого
                    # обмена; тест на это стоит в
                    # tests/test_topics_assignment_db.py.
                    try:
                        from app.modules.topics.service import TopicsService

                        await TopicsService.assign_after_indexing(session, doc_id)
                    except Exception:
                        logger.exception(
                            "Слой тем упал на документе %s; индексация не "
