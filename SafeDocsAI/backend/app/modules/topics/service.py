...
# Умолчание указывает на модель ВТОРОЙ ВОЛНЫ — обученную на таджикских
# документах, а не на синтетике и Википедии.
#
# Что было раньше и почему сменилось. task1_multilingual_dataset/
# topic_model_best.npz учился на 840 синтетических корпоративных текстах и 1438
# выдержках Википедии; тем «Законодательство» и «Послания» в нём не было вовсе.
# Замер на боевых документах: пять посланий президента — один автор, один жанр —
# разъехались по двум темам, причём запас уверенности у них был 0.002–0.022
# против 0.138 у документа, определённого верно. Модель не ошибалась, она
# бросала монетку.
#
# topics_tj/topic_model_tj.npz учится на 2741 таджикском документе с
# редакционными рубриками — тот же жанр и язык, что у документов системы. На тех
# же пяти паёмах: все пять в одной теме «Паём ва баромадҳои Президент», запас
# 0.26–0.32. Разбор — docs/kmeans-topics.md.
#
# Прежние артефакты остаются на диске: они нужны для сравнения в отчёте, но не
# для боевой разметки.
DEFAULT_TOPIC_MODEL_PATH = "data/topics_tj/topic_model_tj.npz"
...
_METRIC_ALIASES = {
    ...
    # Имена с _rubric_ — вторая волна (cluster_topics_tj.py). Там разметка
    # приходит не из выдуманной таксономии, а из редакционных рубрик сайтов, и
    # называть её «темой» было бы неточно. Псевдоним, а не переименование в
    # обучающем скрипте: сравнивается с рубриками ровно то же, что раньше
    # сравнивалось с темами, и в карточке модели это одна и та же строка.
    "ari_topic": (
        "ari_topic",
        "ari_topic_test",
        "ari_rubric_test",
    ...
    "purity": ("purity", "purity_topic_test", "purity_rubric_test"),
