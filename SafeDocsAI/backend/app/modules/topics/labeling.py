...
# В какой доле ДОКУМЕНТОВ кластера слово должно встречаться, чтобы попасть в
# подпись.
#
# Мера c-TF-IDF отвечает на вопрос «чем этот кластер отличается от прочих» — и
# отвечает верно. Но подпись отвечает на другой вопрос: «что в этом кластере
# лежит», а это не одно и то же.
#
# Замер на живом корпусе: кластер из сотни документов, куда легли шесть законов
# об энергетике и десяток статей про автомобили, получил имя «Тематическая
# группа — porsche, nissan». Слово «porsche» стоит в двух документах из ста, и
# именно поэтому оно самое характерное: больше нигде его нет. А кластер с
# законом о финансах назвался «гамбино, готти» — по двум статьям про мафию.
#
# Требование встречаться в пятой части документов убирает такие слова целиком:
# то, что есть в двух документах из ста, кластер не описывает, каким бы редким
# оно ни было.
MIN_DOCUMENT_SHARE = 0.15

# Оба числа подобраны ЗАМЕРОМ, а не на глаз, и подбирались вместе: они тянут в
# разные стороны, и между ними остаётся щель, из которой берутся слова подписи.
# Слишком узкая щель — и кластер получает номер вместо имени.
#
# Перебор шестнадцати пар на живом корпусе (20 кластеров):
#
#   потолок 0.5, пол 0.05 -> 20 имён, но слова «автомобиля, тока», «иисуса, ибн»
#   потолок 0.5, пол 0.20 -> только 10 имён из 20, половина папок без названия
#   потолок 0.7, пол 0.15 -> 20 имён, слова «квт, электроэнергии», «книги,
#                            традиции» — то есть описывающие содержимое
#   потолок 0.8, пол 0.20 -> 20 имён, но слова снова общие: «виде», «модель»
#
# Взята середина: все папки названы, и названы по делу.
...
def c_tf_idf(
    ...
    *,
    ...
    # В скольких документах СВОЕГО кластера встречается слово. Считается по
    # groups, а не по reference: вопрос здесь про содержимое этого кластера, а
    # не про редкость слова вообще.
    in_documents: dict[Hashable, Counter] = {}
    for key, documents in groups.items():
        seen_here: Counter = Counter()
        for tokens in documents:
            seen_here.update(set(tokens))
        in_documents[key] = seen_here
    ...
    for key, counter in counts.items():
        ...
        for term, count in counter.items():
            ...
            if len(reference) > 1 and spread[term] > ceiling:
                continue
            # Слово из двух документов сотни кластер не описывает, каким бы
            # редким оно ни было. Порог применяется только там, где документов
            # достаточно, чтобы доля вообще что-то значила.
            documents_here = len(groups.get(key, ()))
            if documents_here >= 5:
                floor = max(2, int(documents_here * MIN_DOCUMENT_SHARE))
                if in_documents[key][term] < floor:
                    continue
            tf = count / size
