...
# Сколько секций подряд план имеет право разметить ОДНОЙ раскладкой.
#
# Число стоит ЗДЕСЬ, потому что им отвергается ответ модели
# (PresentationPlan._check_layout_runs ниже), а всё, чем схема отвергает ответ,
# по общему правилу модуля живёт в схеме; промпт читает то же самое значение,
# переэкспортированное через constants.py. Двумя числами эта пара уже была —
# просьба в промпте и ничего в схеме, — и разошлась ровно так, как и должна была.
#
# Почему проверка, а не просьба. Требование «не больше трёх подряд» жило в
# тексте план-промпта, и живой прогон показал, чего оно стоит: из восьми
# содержательных секций колоды нестандартную раскладку получила одна, то есть
# семь списков подряд при заявленном пределе три. Счётчик расхождений при этом
# показал ноль — слайды исполнили назначенное, однообразие пришло из плана.
# Правило, которое некому проверить, — не механизм, а пожелание. Отвергнутый
# план уходит в уже существующий повтор (call_with_one_retry, LLM_CALL_ATTEMPTS),
# и текст отказа ниже написан так, чтобы вторая попытка знала, что исправлять.
#
# Тройка, а не двойка. В колоде минимальной длины (SLIDE_COUNT_MIN = 5,
# constants.py) содержательных секций ровно три, и предел в две сделал бы
# честный план из трёх списков невозможным — то есть проверка запрещала бы не
# лень, а короткий заказ.
#
# И главное: предел НЕ требует выдумывать форму. У модели всегда остаётся
# честный выход — quote: дословная формулировка есть в любом документе, в
# отличие от второй стороны сравнения или центрального числа, которых в
# материале может не быть вовсе. Поэтому и текст отказа, и правило промпта
# называют quote прямо: без него требование разнообразия толкает модель
# сочинить сравнение, то есть соврать про документ, а это хуже однообразия.
PLAN_LAYOUT_RUN_MAX = 3
...
class PresentationPlan(BaseModel):
    ...
    @classmethod
    def _check_layout_runs(cls, sections: list[PlanSection]) -> list[PlanSection]:
        """Не больше PLAN_LAYOUT_RUN_MAX одинаковых раскладок подряд.

        Проверка стоит в СХЕМЕ ПЛАНА, а не в сервисе, по двум причинам. Первая:
        отказ схемы уже подключён к повтору — сервис отдаёт модели error_text и
        просит переразметить (call_with_one_retry), и никакой новой машинерии
        для этого правила заводить не надо. Вторая: список секций плана — это
        ровно то место, где серия вообще существует. Слайд-вызов видит одну
        секцию и про соседние ничего не знает.

        Считаются только СОДЕРЖАТЕЛЬНЫЕ секции — и это выходит само, потому что
        других в плане нет: титул и «Источники» рисует рендерер
        (RENDERER_ADDED_SLIDES), раскладок у них не бывает, и в sections они не
        попадают. Проверять здесь нечего исключать.

        Отказ называет ВСЕ нарушенные серии, а не первую. Попытка после отказа
        ровно одна (LLM_CALL_ATTEMPTS = 2): план, исправленный в одном месте из
        двух, сгорит вместе с заказом, и претензия обязана перечислить всё.
        """
        runs: list[str] = []
        start = 0
        for index in range(1, len(sections) + 1):
            if index < len(sections) and (
                sections[index].layout == sections[start].layout
            ):
                continue
            length = index - start
            if length > PLAN_LAYOUT_RUN_MAX:
                runs.append(
                    f"{sections[start].layout!r} in sections.{start}-{index - 1} "
                    f"({length} in a row)"
                )
            start = index
        if runs:
            # Текст — единственная подсказка второй попытки, поэтому в нём три
            # факта (какая раскладка, сколько подряд, какой предел) и выход:
            # «переразметь по материалу» без названного запасного варианта
            # читается как «выдумай сравнение», а выдуманное сравнение хуже
            # однообразной колоды.
            raise ValueError(
                f"a layout repeats more than {PLAN_LAYOUT_RUN_MAX} sections in a "
                f"row: {'; '.join(runs)}. Re-mark a section inside each run by ITS "
                f"material, or mark it {LAYOUT_QUOTE!r}: a wording that matters "
                "literally is in every document, unlike a second side or a central "
                "number."
            )
        return sections
