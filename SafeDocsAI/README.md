...
Поддерживаются PDF, DOCX, TXT и FB2 (до 50 МБ на файл). FB2 загружается
без ZIP-обёртки: приложение извлекает текст книги, глав, таблиц и примечаний,
индексирует его для чата и показывает текстовый предпросмотр. Исходный FB2
можно получить через `/api/v1/sources/{id}/preview?download=true`.
Кодировка читается из XML; DTD и внешние сущности запрещены.
