...
const ERROR_CODE_KEYS = {
    ...
    // Кластеризация по темам. Свой префикс и свои ключи: «модель тем не
    // обучена» — не ошибка запроса и не отказ в правах, действие по ней у
    // пользователя одно (подождать администратора), и общий текст про источники
    // тут врал бы о причине.
    //
    // model_missing приходит с 404, но пустым экраном не оборачивается: раздел
    // «Темы» разбирает этот код сам и показывает состояние «темы ещё не
    // размечены». Перевод остаётся здесь на случай, когда тем же кодом ответит
    // любая другая ручка тем.
    'topic.model_missing': 'topics.errors.modelMissing',
    // 409: переразметка уже идёт. Повторный запуск не нужен и не возможен —
    // это ответ о состоянии работы, а не отказ.
    'topic.reassign_in_progress': 'topics.errors.reassignInProgress',
    // Считать темы нечем: модель эмбеддингов недоступна. Отдельно от
    // settings.embedding_model_unset — там модель не выбрана и чинит это админ
    // в настройках, здесь она выбрана, но не отвечает.
    'topic.embedding_unavailable': 'topics.errors.embeddingUnavailable',
