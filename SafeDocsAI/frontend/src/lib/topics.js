// Общие правила темы документа: как читается подпись, как темы упорядочены и
// как фильтр по теме переносится между экраном «Темы» и списком источников.
//
...
// Доли темы (topic.share) здесь нет намеренно. Процент на экране отвечал на
// вопрос, которого никто не задавал: пользователю важно, что лежит в папке, а
// не какую часть базы она занимает, а администратору размеры тем видны по
// количеству. Поле в ответе API осталось — его просто никто не читает.
 ...
 *
 * Раскладка общая, а поведение — нет, и это не рассогласование. В разделе
 * «Темы» в папку ВХОДЯТ: экран сменяется её содержимым, как в проводнике. В
 * выборе источников папка раскрывается на месте, потому что уводить человека
 * с экрана посреди расстановки галочек значит потерять уже отмеченное.
 */
 ...
 *
 * Порог нужен одному экрану — выбору источников для блокнота: только там папки
 * раскрываются на месте. В разделе «Темы» в папку входят, и раскрывать нечего.
 */
