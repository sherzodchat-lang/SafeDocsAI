 ...
 * снимались замеры времени вызова (LLM_CALL_TIMEOUT в constants.py).
 *
 * От времени генерации это число больше не зависит: потолок джобы выводится
 * из slide_count функцией presentation_job_timeout, а не назначен под одну
 * длину колоды. Менять умолчание можно из соображений интерфейса.
 */
...
/**
 * Заголовок колоды — то, О ЧЁМ она.
 *
 * Приходит полем title (backend: presentation.title) и рождается не там же, где
 * description: описание пишет пользователь ДО генерации как пожелание, а
 * заголовок формулирует модель ПОСЛЕ неё по найденному материалу. Поэтому
 * подставлять сюда описание нельзя ни при каких обстоятельствах — это разные
 * утверждения о колоде, и подмена читалась бы как «модель поняла задачу
 * дословно».
 *
 * Пусто заголовок бывает у трёх видов строк, и запасная подпись у них тоже
 * разная — её выбирает вызывающий и передаёт сюда:
 *   * заказ ещё в очереди или генерируется — заголовка ЕЩЁ не существует;
 *   * генерация упала — его не будет никогда;
 *   * колода собрана до появления колонки, и восстановить его из файла не
 *     вышло (.pptx прежнего рендерера, потерянный файл).
 *
 * Выдумывать подпись здесь нечем и не нужно: неверная, но правдоподобная
 * подпись хуже честной пометки «названия пока нет».
 */
export const resolvePresentationTitle = (presentation, fallback = '') => {
