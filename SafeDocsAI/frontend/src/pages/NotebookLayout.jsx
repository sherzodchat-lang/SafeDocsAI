 ...
 *
 * Вкладок «Заметки» и «Источники» здесь нет: обе дублировали панели «Обзора».
 * Страница заметок была беднее панели — ни правки, ни архивации, ни удаления;
 * вкладка источников, наоборот, повторяла общую таблицу «Все источники» рядом с
 * полосой, которая умеет меньше. В обоих случаях это две точки работы с одними
 * и теми же данными — повод гадать, где работать. Повседневное — открыть,
 * загрузить, привязать, удалить — делает полоса в «Обзоре», а за сортировкой,
 * фильтрами и страницами она ведёт одной ссылкой на общую страницу источников.
 */
const NOTEBOOK_TABS = [
...
const NotebookLayout = () => {
  ...
  useEffect(() => {
    ...
    // Пользовательской роли остаётся один «Обзор»: полоса из единственной
    // вкладки никуда не ведёт, поэтому её не рисуем вовсе.
    if (items.length < 2) {
      setNotebookTabs(null);
      return undefined;
    }
